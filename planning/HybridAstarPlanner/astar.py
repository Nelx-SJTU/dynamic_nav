...
def design_obstacles_cv2(obstacle_map, env_size=2000):
    ox, oy = [], []
    for i in range(env_size):
        for j in range(env_size):
            if obstacle_map[i][j] < 127:
                ox.append(i)
                oy.append(j)
    return ox, oy
...
def main():
    ...
    # ox, oy = get_env()
    map_t = "D:/Document/Programme/ChargingPile/Planning/images/img_128/map_19.png"
    map_t = cv2.imread(map_t, cv2.IMREAD_GRAYSCALE)
    ox, oy = design_obstacles_cv2(map_t)
    print("ready")
